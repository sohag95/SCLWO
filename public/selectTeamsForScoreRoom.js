...
function getGroupNames(){
  ...
  if(tournamentData.isSecondRoundStarted){
    secondRoundName.value=tournamentData.secondRoundName
...
function getFirstTeamName(selectedGroup){
  allTeamNames=[]
      let tournamentGroups
      if(tournamentData.isSecondRoundStarted){
        tournamentGroups=tournamentData.secondRoundGroups
...
groupPart.addEventListener("change", () => {
  ...
  secondTeamPart.innerHTML=""
  if(selectedGroup){
    let firstTeams=getFirstTeamName(selectedGroup)
    let tag=document.createElement("option")
    tag.value=""
    tag.innerHTML="--Select First Team Name--"
    firstTeamPart.options.add(tag)
